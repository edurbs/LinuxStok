...
class devolucoesdetalhadasdocliente extends relatorios {

    function devolucoesdetalhadasdocliente(){
		...
		$this->janela->set_title("Relatorio de Devolucoes Detalhado por Item");
	...
	function gerar($tipo){
		...
		$sql="SELECT s.vendedor, f.nome, s.data, s.hora, s.codcli, c.nome, e.codmerc, m.descricao, e.precocomdesconto as precounitario, e.quantidade, e.precocomdesconto*e.quantidade as precototal FROM entsai AS e INNER JOIN devolucoes AS s ON (e.codentsai=s.coddevolucoes) INNER JOIN clientes AS c ON (s.codcli=c.codigo) INNER JOIN mercadorias AS m ON (e.codmerc=m.codmerc) INNER JOIN funcionarios AS f ON (s.vendedor=f.codigo) WHERE e.tipo='D' AND s.data>='$this->data1' AND s.data<='$this->data2' ";
		if (!empty($codcliente)){
		...
		$titulo="Relatorio de Devolucoes Detalhado por Item";
