...
class caixa_periodo extends relatorios {

    function caixa_periodo (){
	...
	function gerar($tipo){
        ...
        if(empty($this->codigocaixa) or !$this->retornabusca2('cadcaixa', $this->entry_codigo1, $this->label_codigo1, 'codigo', 'descricao')){
		...
		$con=$this->conecta();
		$sql="SELECT c.data, c.hora, c.formamovim, c.valor, c.saldo, c.historico, c.obs FROM caixa AS c WHERE c.data>='$this->data1' AND c.data<='$this->data2' AND c.origem='$this->codigocaixa' ";
		...
		$titulo="Relatorio de Movimento do Caixa";
