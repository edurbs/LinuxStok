...
class relgeral extends funcoes {
	...
	function simplescomendereco($tabela){
        ...
        $con->Connect();
        $sql="SELECT c.nome, e.endereco, e.numero, e.complemento, e.bairro, e.cidade, e.estado, e.cep, e.telefone, e.fax, e.celular FROM $tabela AS c LEFT JOIN cadastro2enderecos AS e ON (e.cadastro='$tabela' AND e.codigo=c.codigo) ORDER BY c.nome";
